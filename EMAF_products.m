function [required, alternatives, all_sets] = EMAF_products(model, constraints, exchangeRxns)
...
solutions = {};
solutions_rxns = {};
solutions{1} = find(solution.int==1);
...
while strcmp(solution.origStat,'OPTIMAL') && length(find(solution.int==1))==lengthSolutions
    ...
    bilevelMILPproblem.csense = csense_bl;
    solution = solveCobraMILP(bilevelMILPproblem);
    if strcmp(solution.origStat,'OPTIMAL') && length(find(solution.int==1))==lengthSolutions
        solutions{end+1} = find(solution.int==1);

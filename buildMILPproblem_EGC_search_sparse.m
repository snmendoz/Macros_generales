function bilevelMILPproblem = buildMILPproblem_EGC_search_sparse(models, binary_variable_ids, constraints, previousSolutions, penalizedReactions, penalizationFactors, nmax)
...
A_int_constraints = sparse(1,n_variables_total);
A_int_constraints(sel_int_sol) = ones(1,length(sel_int_sol));
A_bl = [A_bl; A_int_constraints];
b_bl = [b_bl; nmax];
csense_bl(end+1) = 'L';

%% addition of previous solutions

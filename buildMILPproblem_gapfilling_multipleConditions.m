function bilevelMILPproblem = buildMILPproblem_gapfilling_multipleConditions(models, binary_variable_ids, constraints, previousSolutions)
...
for i = 1:n_rxns_int
    for j = 1:length(models)
        ...
        A_int_constraints(cont,pos) = H;
        A_int_constraints(cont,n_rxn_total+i) = -H*ub(pos);
    end
end
...
if ~isempty(previousSolutions)
    ...
    lengthSolutions = length(previousSolutions{1});
    ...
    for i = 1:length(previousSolutions)
        ...
        A_newSolution = zeros(1,n_rxn_total+n_rxns_int);

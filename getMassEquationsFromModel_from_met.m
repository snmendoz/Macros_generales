function [massBalances,massBalances2] = getMassEquationsFromModel_from_met(model, pos)

if ~isfield(model,'csense')
   warning('no csense field. Assuming equalities');
...
massBalances2 = cell(length(pos),1);
variables = strcat('v_', model.rxnNames);
for i = 1:length(pos)
     pos_variables_equation_i = find(model.S(pos(i),:));
     coefs = full(model.S(pos(i),pos_variables_equation_i));
     equation_i = [model.mets{pos(i)} ': '];

function [rxnsProducingCompound, fluxRxnsProducingCompound] = getRxnsProducingCompound_FVA(model,compound, FBAsolution)
...
posRxnsProducingCompound = union(intersect(find(isReactant), find(canReactionBeBackward)), intersect(find(~isReactant), find(canReactionBeForward)));
rxnsProducingCompound = rxns(posRxnsProducingCompound);
fluxRxnsProducingCompound = FBAsolution(posRxns(posRxnsProducingCompound));

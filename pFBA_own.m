function [GeneClasses, RxnClasses, modelIrrevFM, solutionpFBA] = pFBA_own(model, varargin)
...
if skipclass % skip the model reduction and gene/rxn classification
    ...
    tol = 10^-6;
    model.lb(find(model.c==1)) = FBAsoln.x(find(model.c==1))-tol;
    [ MinimizedFlux, modelIrrevFM, solutionpFBA ]= minimizeModelFlux_local(model,GeneOption);
...
function [ MinimizedFlux, modelIrrev, solution_rev]= minimizeModelFlux_local(model,GeneOption)

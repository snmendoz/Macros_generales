function newModel=ravenCobraWrapper_own(model)
% ravenCobraWrapper
        ...
        for i=1:numel(model.mets)
            ...
            if isfield(model,'metChEBIID')
                if ~isempty(model.metChEBIID{i})
                    ids = strsplit(model.metChEBIID{i},'; ');
                    for j = 1:length(ids)
                        newModel.metMiriams{i,1}.name{counter,1} = 'chebi';
                        newModel.metMiriams{i,1}.value{counter,1} = ids{j};
                        counter=counter+1;
                    end
                end
            end
            ...
            if isfield(model,'metKEGGID')
                if ~isempty(model.metKEGGID{i})
                    ids = strsplit(model.metKEGGID{i},'; ');
                    for j = 1:length(ids)

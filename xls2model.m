...
if isempty(find(cellfun(@isempty, metCompAbbrev)==0))
   metCompAbbrev = cellfun(@(x) regexp(x,'.*_(.*)$','tokens'), mets, 'UniformOutput', 0); 
   mets = regexprep(mets,'(.*)_(.*)$' ,'$1\[$2\]');
end
...
if isempty(strmatch('Compartment',metHeaders,'exact'))
    ...
    Compartments = MetStrings(B(A)+1,strmatch('Compartment',metHeaders,'exact'));
    Cytosolname = 'cytosol';
...
if ~isempty(strmatch('Description',metHeaders,'exact'))
    model.metNames = columnVector(MetStrings(B(A)+1,strmatch('Description',metHeaders,'exact')));
end
%%Set Formulas
if ~isempty(strmatch('Charged formula',metHeaders,'exact'))
    model.metFormulas = columnVector(MetStrings(B(A)+1,strmatch('Charged formula',metHeaders,'exact')));
end
if ~isempty(strmatch('Formula',metHeaders,'exact'))
    model.metFormulas = columnVector(MetStrings(B(A)+1,strmatch('Formula',metHeaders,'exact')));
end
%%Set Charge
if ~isempty(strmatch('Charge',metHeaders,'exact'))
    model.metCharges = cell2mat(columnVector(metInfo(B(A)+1,strmatch('Charge',metHeaders,'exact'))));
end

if ~isempty(strmatch('SMILES',metHeaders,'exact'))
    model.metSmiles= columnVector(MetStrings(B(A)+1,strmatch('SMILES',metHeaders,'exact')));
end

%% Set annotations. (Has to be updated, once annotation structure is defined)
if ~isempty(strmatch('KEGG ID',metHeaders,'exact'))
    model.metKEGGID = columnVector(MetStrings(B(A)+1,strmatch('KEGG ID',metHeaders,'exact')));
end
if ~isempty(strmatch('InChI string',metHeaders,'exact'))
    model.metInChIString = columnVector(MetStrings(B(A)+1,strmatch('InChI string',metHeaders,'exact')));
end
if ~isempty(strmatch('HMDB ID',metHeaders,'exact'))
    model.metHMDBID = columnVector(MetStrings(B(A)+1,strmatch('HMDB ID',metHeaders,'exact')));
end
...
if ~isempty(strmatch('PubChem ID',metHeaders,'exact'))
    ...
    model.metPubChemID = columnVector(metInfo(B(A)+1,strmatch('PubChem ID',metHeaders,'exact')));
...
if ~isempty(strmatch('ChEBI ID',metHeaders,'exact'))
    model.metChEBIID  = columnVector(metInfo(B(A)+1,strmatch('ChEBI ID',metHeaders,'exact')));
